...
**************************************************************************************
 ...
 Le mode d'automation des volets est choisi grâce à des selector switches (Virtual Switch) à créer dans Domoticz.
    ...
    - Mode 'BioClim'. Ouverture/Fermeture partielle des volets par plages horaires.
        ...
        - 1 ou 2 tables peuvent être définies pour chaque groupe. Le choix de la table de configuration peut être
        automatique en fonction d'un seuil de température ou forcé à une valeur par un selector switch (optionnel).
        - Le mode 'BioClim' est validé pour le printemps et l'été, voir variable BIOCLIM_SEASONS.
        - Optionnellement le niveau d'ouverture est adapté en fonction de la température extèrieure.
 ...
 -------------
    ...
    4) Copier les fichiers suivants dans /home/pi/domoticz/scripts/lua/ :
        ...
        - fonctions.lua
        - includes.lua
        - script_time_Automation_Volets_V1.2.lua
]]
...
--  Labels pour les selector switches de contrôle des groupes de volets.
--      0:'Manuel', 10:'Jour/Nuit', 20:'BioClim', 30:'Auto'.
--      Reporter ces labels dans MODE_MANUAL, MODE_ONOFF, MODE_BIOCLIM et MODE_AUTO.
--
MODE_MANUAL     = 'Manuel'      
...
-- SW_CLOSING_MODE, nom du switch On/Off virtuel.
...
SW_CLOSING_MODE    = 'Fermeture mode Aération'

-- SECURITY_SWITCH, nom du selector switch virtuel.
--  Pour le choix du niveau sécurité.
--	Nom: nom défini dans Domoticz, ex 'Sécurité'.
--  La valeur sera 'Off' si le switch n'existe pas.
SECURITY_SWITCH		= 'Sécurité'
--	Valeurs : 
--		Nom: libre, ex 'Manuel', 'BioClim'. 
--		ex valeurs:  0:'Inactive', 10:'Intérieure', 20:'Générale'.
SECURITY_OFF		= 'Inactive'		
SECURITY_INSIDE		= 'Intérieure'	
SECURITY_GLOBAL 	= 'Générale'

--
-- SW_AUTOMATION, nom du switch On/Off virtuel.
--  Contrôle générale de l'automation, voir Config_Automation_Common.lua.
--  La valeur sera 'On' si le switch n'existe pas.
SW_AUTOMATION   = 'Automation'
...
--  Mesure de la luminosité extèrieure, voir Config_Automation_Common.lua.
...
-----------------------------------------------------------------------------------------------
-- GROUPES D'UNITÉS POUR LE MODE ON/OFF. 
--
...
-- Groupe_Etage = {'Volet Chambre Verte', 'Volet Bureau'}
...
--        L'utilisation de 2ème paramètres est contrôlée par le switch SW_CLOSING_MODE.
-------------------------------------------------------------------------------------------
Group_OnOff = {} 
...
--  Les valeurs sont les noms des selector switches qui contrôlent chaque groupe.
----------------------------------------------------------------------------------------
Groups_BioClim = {} 
